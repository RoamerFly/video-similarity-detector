...
从 v1.1.0 开始，Windows、macOS 和 Linux 的应用本体、安装包、便携包与 updater 资产都不再携带 Python、PyTorch、CUDA 和 FFmpeg。运行环境作为版本化资产独立发布，只在首次启动或 `runtime-version` 变化时下载一次，并安装到 Tauri 管理的应用本地数据目录。
...
应用优先使用与 `desktop/runtime-version.txt` 和当前构建匹配的托管环境。Windows 分 CPU/GPU 两种环境，macOS/Linux 使用各自架构的 CPU 环境。
...
旧版本安装目录中的 `env` 若仍保留在当前可执行文件旁，可作为兼容回退。设置页会优先把它直接移动到应用数据目录；跨磁盘时先复制、校验并原子启用，再删除旧目录，全程不重新下载。若旧目录因权限问题未能删除，用户可在托管环境就绪后单独重试清理。
...
压缩包顶层必须包含 `env/`。每个 runtime 都先下载 `.sha256`，再断点续传单个 ZIP；校验通过后才安全解压到临时目录，并用版本目录原子替换。Windows GPU runtime 固定使用 PyTorch 2.9.1+cu130，构建任务会拒绝超过 GitHub 2 GiB 限制的资产。macOS/Linux 解压时会恢复 ZIP 中的 Unix 权限，确保 Python、FFmpeg 和 FFprobe 可执行。

CLIP 模型继续作为独立资产，由模型安装器下载到 `assets/models`，不会进入应用包或 runtime ZIP。
...
6. 已安装旧版 `env` 的用户可以继续运行，并能从设置页无下载迁移和清理。
