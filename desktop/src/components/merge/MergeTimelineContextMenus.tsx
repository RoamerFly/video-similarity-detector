...
import {
  ArrowRight, Copy, FolderOpen, ListOrdered, Play, Plus,
...
interface MergeTimelineContextMenusProps {
  ...
  onClipPosition: (clip: ClipContextMenuState) => void
...
export function MergeTimelineContextMenus(props: MergeTimelineContextMenusProps) {
  ...
  const { t } = useI18n()
  const newTrackLabelKey = (kind: TimelineTrackKind) => kind === 'video' ? '新建视频线' : kind === 'audio' ? '新建音频线' : '新建文本线'
  const deleteTrackLabelKey = (kind: TimelineTrackKind) => kind === 'video' ? '删除当前视频线' : kind === 'audio' ? '删除当前音频线' : '删除当前文本线'
  return <>
    {props.track && createPortal(<Translated><div className="video-context-menu clip-context-menu track-context-menu" style={clampContextMenuPosition(props.track.x, props.track.y, viewport.width, viewport.height, 220, 120)} role="menu" onPointerDown={(event) => event.stopPropagation()}>
      {props.track.kind === 'text' && <button type="button" role="menuitem" onClick={() => props.onTrackAddText(props.track!)}><Type />{t('添加文本片段')}</button>}
      <button type="button" role="menuitem" onClick={() => props.onTrackAdd(props.track!.kind)}><Plus />{t(newTrackLabelKey(props.track.kind))}</button>
      <button className="danger" type="button" role="menuitem" disabled={props.trackCount[props.track.kind] <= 1} title={t('删除轨道后，其中的片段会移动到保留的第一条同类轨道')} onClick={() => props.onTrackRemove(props.track!)}><Trash2 />{t(deleteTrackLabelKey(props.track.kind))}</button>
    ...
    {props.clip && createPortal(<Translated><div className="video-context-menu clip-context-menu" style={{ ...clampContextMenuPosition(props.clip.x, props.clip.y, viewport.width, viewport.height, 250, 390), maxHeight: Math.max(160, viewport.height - props.clip.y - 8) }} role="menu" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}>
      ...
      <button type="button" role="menuitem" onClick={() => props.onClipSeek(props.clip!)}><SkipBack />{t('定位到右键位置')}</button><button type="button" role="menuitem" onClick={() => props.onClipPlay(props.clip!)}><Play />{t('从片段开头播放')}</button><button type="button" role="menuitem" disabled={!props.canSplit} onClick={() => props.onClipSplit(props.clip!)}><Scissors />{t('在右键位置拆分')}</button><button type="button" role="menuitem" onClick={() => props.onClipExtractAudio(props.clip!)}><Volume2 />{t('提取该片段音频')}</button>
      <button type="button" role="menuitem" onClick={() => props.onClipToggleMute(props.clip!)}>{props.clip.layout.item.muted ? <Volume2 /> : <VolumeX />}{t(props.clip.layout.item.muted ? '恢复片段原音' : '静音该视频片段')}</button><button type="button" role="menuitem" onClick={() => props.onClipRotate(props.clip!)}><RotateCw />{t('旋转操作')}</button><button type="button" role="menuitem" onClick={() => props.onClipCrop(props.clip!)}><SquareDashedMousePointer />{t('调整视频尺寸')}</button><button type="button" role="menuitem" onClick={() => props.onClipDuplicate(props.clip!)}><Copy />{t('复制片段')}</button>
      <button type="button" role="menuitem" onClick={() => props.onClipPosition(props.clip!)}><ListOrdered />{t('调整位置')}</button><button type="button" role="menuitem" disabled={!props.canRestoreClip} onClick={() => props.onClipRestore(props.clip!)}><RotateCcw />{t('恢复完整片段')}</button><button type="button" role="menuitem" onClick={() => props.onClipReveal(props.clip!)}><FolderOpen />{t('在文件夹中显示')}</button><button className="danger" type="button" role="menuitem" onClick={() => props.onClipRemove(props.clip!)}><Trash2 />{t('删除片段')}</button>
    </div></Translated>, document.body)}
    {props.audio && createPortal(<Translated><div className="video-context-menu clip-context-menu audio-context-menu" style={{ ...clampContextMenuPosition(props.audio.x, props.audio.y, viewport.width, viewport.height, 240, 270), maxHeight: Math.max(160, viewport.height - props.audio.y - 8) }} role="menu" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}><strong title={props.audio.layout.item.path}>{props.audio.layout.item.name}</strong><span className="clip-context-menu-range">{t('时间线位置')} {props.formatTime(props.audio.layout.start)}</span><button type="button" role="menuitem" onClick={() => props.onAudioSeek(props.audio!)}><SkipBack />{t('定位到音频开头')}</button><button type="button" role="menuitem" onClick={() => props.onAudioMoveToPlayhead(props.audio!)}><ArrowRight />{t('移动到播放头')}</button><button type="button" role="menuitem" disabled={props.audio.layout.start === 0} onClick={() => props.onAudioMoveToStart(props.audio!)}><RotateCcw />{t('移到时间线起点')}</button><button type="button" role="menuitem" onClick={() => props.onAudioEditProperties(props.audio!)}><Settings2 />{t('属性')}</button><button type="button" role="menuitem" onClick={() => props.onAudioReveal(props.audio!)}><FolderOpen />{t('在文件夹中显示')}</button><button className="danger" type="button" role="menuitem" onClick={() => props.onAudioRemove(props.audio!)}><Trash2 />{t('删除音频片段')}</button></div></Translated>, document.body)}
    {props.text && createPortal(<Translated><div className="video-context-menu clip-context-menu text-context-menu" style={{ ...clampContextMenuPosition(props.text.x, props.text.y, viewport.width, viewport.height, 240, 250), maxHeight: Math.max(160, viewport.height - props.text.y - 8) }} role="menu" onPointerDown={(event) => event.stopPropagation()} onWheel={(event) => event.stopPropagation()}><strong title={props.text.text.text}>{props.text.text.text}</strong><span className="clip-context-menu-range">{t('时间线位置')} {props.formatTime(props.text.text.startTime)}</span><button type="button" role="menuitem" onClick={() => props.onTextSeek(props.text!)}><SkipBack />{t('定位到文本开头')}</button><button type="button" role="menuitem" onClick={() => props.onTextMoveToPlayhead(props.text!)}><ArrowRight />{t('移动到播放头')}</button><button type="button" role="menuitem" onClick={() => props.onTextEditProperties(props.text!)}><Settings2 />{t('属性')}</button><button className="danger" type="button" role="menuitem" onClick={() => props.onTextRemove(props.text!)}><Trash2 />{t('删除文本片段')}</button></div></Translated>, document.body)}
  </>
}
