  ...
  return (
    <div className="merge-text-properties-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <section className="merge-text-properties-dialog" role="dialog" aria-modal="true" aria-labelledby="merge-text-properties-title" onPointerDown={(event) => event.stopPropagation()}>
        <header>
          <div>
            <span className="eyebrow">{t('文本样式')}</span>
            <h2 id="merge-text-properties-title">{t('编辑文本属性')}</h2>
          </div>
          <button ref={closeRef} type="button" className="icon-button" aria-label={t('关闭文本属性')} onClick={onClose}><X /></button>
        </header>
        ...
        <div className="merge-text-properties-fields">
          ...
          <label>
            ...
            <div className="merge-text-color-field">
              <input type="color" value={color} aria-label={t('选择文本颜色')} onChange={(event) => { setColor(event.target.value); setColorInput(event.target.value) }} />
              <TextInput value={colorInput} aria-label={t('文本颜色值')} onChange={(event) => { setColorInput(event.target.value); if (isHexColor(event.target.value)) setColor(event.target.value) }} onBlur={() => setColorInput(color)} />
            </div>
          </label>
        </div>
        <footer>
          <button type="button" className="icon-button" onClick={onClose}>{t('取消')}</button>
          <NeonButton type="button" onClick={() => { onSave({ text, fontSize, color }); onClose() }}>{t('保存属性')}</NeonButton>
        </footer>
      </section>
    </div>
  )
