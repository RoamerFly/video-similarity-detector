...
export function MergePage() {
  ...
  const transitioningClipRef = useRef('')
  ...
  const [dragTargetClipId, setDragTargetClipId] = useState('')
  ...
  useEffect(() => {
    if (!clipContextMenu && !audioContextMenu) return undefined
    const close = () => {
  ...
  }, [audioContextMenu, clipContextMenu])
  ...
  const clipLayouts = useMemo(() => {
    return merge.items.reduce<ClipLayout[]>((layouts, item) => {
      const cursor = layouts.at(-1)?.end ?? 0
      const info = metadata[normalizePath(item.path)]
      const duration = clipDuration(item, info)
      const layout = { item, start: cursor, duration, end: cursor + duration }
      return [...layouts, layout]
    }, [])
  }, [merge.items, metadata])
  const probing = merge.items.some((item) => !metadata[normalizePath(item.path)])
  const totalDuration = clipLayouts.at(-1)?.end ?? 0
  ...
  const currentLayout = findLayoutAt(clipLayouts, playhead) ?? selectedLayout
  ...
  const resolutionOptions = useMemo(() => {
    ...
    const maxOption = {
      label: `时间线最大 ${resolutionBounds.width} × ${resolutionBounds.height}`,
      width: resolutionBounds.width,
      height: resolutionBounds.height,
    }
    if (!options.some((item) => item.width === maxOption.width && item.height === maxOption.height)) {
      options.unshift(maxOption)
    }
    return options
  ...
  const outputWidth = merge.settings.width
  const outputHeight = merge.settings.height
  const setMergeSettings = merge.setSettings

  useEffect(() => {
    if (!resolutionBounds.ready) return
    const width = evenDimension(clamp(outputWidth, minimumOutputDimension, resolutionBounds.width))
    const height = evenDimension(clamp(outputHeight, minimumOutputDimension, resolutionBounds.height))
    if (width !== outputWidth || height !== outputHeight) {
      setMergeSettings({ width, height }, false)
    }
  }, [
    outputHeight,
    outputWidth,
    resolutionBounds.height,
    resolutionBounds.ready,
    resolutionBounds.width,
    setMergeSettings,
  ])
  ...
  const scrubGlobal = useCallback((time: number, forceMediaUpdate = false) => {
    ...
    const layout = findLayoutAt(clipLayouts, next) ?? clipLayouts.at(-1) ?? null
    playheadRef.current = next
    setPlayhead((current) => Math.abs(current - next) >= 0.0005 ? next : current)
    if (layout) setSelectedClipId((current) => current === layout.item.id ? current : layout.item.id)

    const video = previewRef.current
    if (!video || !layout || video.dataset.clipId !== layout.item.id || video.readyState < 1) return
    const now = Date.now()
  ...
  useEffect(() => {
    ...
    const layout = clipLayouts.find((item) => item.item.id === previewClip.id)
    if (!layout) return
    const video = previewRef.current
    if (!video) return
    const target = previewClip.trimStart + Math.max(0, playheadRef.current - layout.start)
    const apply = () => {
  ...
  }, [clipLayouts, playing, previewClip])

  const syncPlayheadFromPreview = useCallback(() => {
    const video = previewRef.current
    ...
    const globalTime = clamp(
      layout.start + Math.max(0, video.currentTime - layout.item.trimStart),
      layout.start,
      layout.end,
    )
    playheadRef.current = globalTime
    setPlayhead((current) => Math.abs(current - globalTime) >= 0.0005 ? globalTime : current)
    keepTimelineTimeVisible(globalTime)

    if (video.currentTime < sourceEnd - 0.001 || transitioningClipRef.current === layout.item.id) return
    transitioningClipRef.current = layout.item.id
    const nextIndex = clipLayouts.findIndex((item) => item.item.id === layout.item.id) + 1
    if (nextIndex < clipLayouts.length) {
      seekGlobal(clipLayouts[nextIndex].start, true)
      return
    }
  ...
  }, [clipLayouts, keepTimelineTimeVisible, metadata, previewClip, seekGlobal, totalDuration])

  useEffect(() => {
    transitioningClipRef.current = ''
  }, [previewClip?.id])

  useEffect(() => {
    if (!playing || !previewClip) return undefined
    const update = () => {
  ...
  }, [playing, previewClip, syncPlayheadFromPreview])
  ...
  function togglePlayback() {
    ...
    if (playhead >= totalDuration - 0.02) seekGlobal(0, true)
    else void video.play().then(() => setPlaying(true)).catch(() => setPlaying(false))
  }
  ...
  function handlePlayheadHandlePointerDown(event: React.PointerEvent<HTMLButtonElement>) {
    if (event.button !== 0 || !timelineRef.current || totalDuration <= 0 || merge.running) return
    event.preventDefault()
    event.stopPropagation()
    let active = false
    let latestClientX = event.clientX
    const video = previewRef.current
    const resumeAfterDrag = Boolean(video && !video.paused)
    ...
    const end = () => {
      ...
      if (active) {
        ...
        if (resumeAfterDrag) window.requestAnimationFrame(() => void previewRef.current?.play().catch(() => setPlaying(false)))
      }
  ...
  function handleTimelinePointerDown(event: React.PointerEvent<HTMLDivElement>) {
    ...
    const video = previewRef.current
    const resumeAfterSeek = Boolean(video && !video.paused)
    ...
    const end = (pointerEvent: PointerEvent) => {
      ...
      if (resumeAfterSeek) window.requestAnimationFrame(() => void previewRef.current?.play().catch(() => setPlaying(false)))
    }
  ...
  function splitClipAt(layout: ClipLayout, timelineTime: number) {
    ...
    const nextId = merge.splitVideo(layout.item.id, sourceTime)
    if (!nextId) {
  ...
  function moveClip(layout: ClipLayout, direction: -1 | 1) {
    merge.moveVideo(layout.item.id, direction)
    setSelectedAudioId('')
    setSelectedClipId(layout.item.id)
    setClipContextMenu(null)
  }
  ...
  function handleVideoClipPointerDown(event: React.PointerEvent, layout: ClipLayout) {
    if (event.button !== 0 || !timelineRef.current || merge.running) return
    event.preventDefault()
    event.stopPropagation()
    setSelectedAudioId('')
    setSelectedClipId(layout.item.id)
    ...
    const longPressTimer = window.setTimeout(() => {
      ...
      setDragTargetClipId(layout.item.id)
    }, 320)
    ...
    const move = (pointerEvent: PointerEvent) => {
      ...
      if (longPressActive) {
        ...
        const contentRect = timelineRef.current?.getBoundingClientRect()
        const target = contentRect
          ? findLayoutAt(clipLayouts, timelineTimeFromClientX(latestX, contentRect, totalDuration))
          : null
        targetId = target?.item.id ?? layout.item.id
        setDragTargetClipId(targetId)
        return
      }
    ...
    const end = (pointerEvent: PointerEvent) => {
      ...
      if (longPressActive) {
        if (targetId !== layout.item.id) merge.moveVideoTo(layout.item.id, targetId)
        setDraggedClipId('')
        setDragTargetClipId('')
        return
      }
      ...
      if (scrubbed && resumeAfterSeek) window.requestAnimationFrame(() => void previewRef.current?.play().catch(() => setPlaying(false)))
    }
  ...
  function handleAudioPointerDown(event: React.PointerEvent, audio: MergeAudioItem) {
    ...
    const move = (pointerEvent: PointerEvent) => {
      ...
      animationFrameRef.current = window.requestAnimationFrame(() => {
        ...
        merge.updateAudio(audio.id, { startTime: clamp(next, 0, totalDuration) }, false)
      })
    }
    const end = () => {
      window.clearTimeout(longPressTimer)
    ...
    <div className={`route-fill merge-editor-page ${dropActive ? 'drop-active' : ''}`}>
      <GlassPanel className="editor-toolbar">
        <div className="editor-toolbar-group">
          <NeonButton variant="outline" type="button" onClick={() => void chooseVideos()} disabled={merge.running}>
            <Plus size={16} />添加视频
          </NeonButton>
          <NeonButton variant="outline" type="button" onClick={() => void chooseAudio()} disabled={merge.running}>
            <Music2 size={16} />导入音频
          </NeonButton>
          <button type="button" title="在播放头位置拆分当前片段" disabled={!selectedClip || merge.running} onClick={splitAtPlayhead}>
            <Scissors />拆分
          </button>
          <button
            className="danger"
            type="button"
            title="移除选中的视频或音频片段"
            disabled={merge.running || (!selectedClip && !selectedAudio)}
            onClick={() => {
              if (selectedAudio) {
          ...
          <button type="button" title="撤销上一步编辑" disabled={merge.running || !merge.canUndo} onClick={merge.undo}>
            <Undo2 />撤销
          </button>
          <button type="button" title="重做上一步编辑" disabled={merge.running || !merge.canRedo} onClick={merge.redo}>
            <Redo2 />重做
          </button>
        </div>
        ...
        <GlassPanel className="editor-preview-panel frame-preview-card video-preview-card">
          ...
          <section className="editor-advanced-settings editor-advanced-settings-below-video">
            ...
            <div className="editor-advanced-inline">
              ...
              <NumberField label="帧率" value={merge.settings.fps} min={1} max={120} onChange={(fps) => merge.setSettings({ fps })} />
              <NumberField label="画质 CRF" value={merge.settings.crf} min={0} max={51} onChange={(crf) => merge.setSettings({ crf })} />
              <label>
                <ParameterHint label="输出分割" tip="可按时长或数量拆成多个输出文件。" />
        ...
        <GlassPanel className="editor-inspector-panel">
          ...
          <div className="editor-output-settings">
            ...
            <div className="editor-resolution-fields">
              <NumberField
                label={`宽度（${minimumOutputDimension}–${resolutionBounds.width}）`}
                value={merge.settings.width}
                min={minimumOutputDimension}
                max={resolutionBounds.width}
                step={2}
                onChange={(width) => merge.setSettings({ width: evenDimension(width) })}
              />
              <NumberField
                label={`高度（${minimumOutputDimension}–${resolutionBounds.height}）`}
                value={merge.settings.height}
                min={minimumOutputDimension}
                max={resolutionBounds.height}
                step={2}
        ...
        <div className="timeline-workspace">
          ...
          <div ref={timelineScrollRef} className="timeline-scroll-viewport" onWheel={handleTimelineWheel}>
            <div
              ...
              style={{ width: timelineContentWidth, minWidth: '100%' }}
            >
              ...
              <div className="timeline-tracks" onPointerDown={handleTimelinePointerDown}>
                  ...
                  {clipLayouts.map((layout) => (
                    <button
                      ...
                      className={[
                        ...
                        dragTargetClipId === layout.item.id && draggedClipId !== layout.item.id ? 'drag-target' : '',
                      ].filter(Boolean).join(' ')}
                      style={{
                ...
                <div className={`timeline-audio-track ${merge.audioItems.length === 0 ? 'empty' : ''}`}>
                  {merge.audioItems.length === 0 && <span className="timeline-empty-hint">拖入音频文件，或右键任意视频片段提取音频</span>}
      ...
      {clipContextMenu && createPortal(
        ...
        >
          ...
          <button
            type="button"
            role="menuitem"
            disabled={merge.items.findIndex((item) => item.id === clipContextMenu.layout.item.id) <= 0}
          ...
          <button
            type="button"
            role="menuitem"
            disabled={merge.items.findIndex((item) => item.id === clipContextMenu.layout.item.id) >= merge.items.length - 1}
            onClick={() => moveClip(clipContextMenu.layout, 1)}
          >
            <ArrowRight />向后移动
          </button>
...
function findLayoutAt(layouts: ClipLayout[], time: number) {
  return layouts.find((layout, index) => time >= layout.start && (time < layout.end || index === layouts.length - 1))
}
