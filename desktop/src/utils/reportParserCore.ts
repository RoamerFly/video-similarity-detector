...
// 纯函数版文件名提取，与 backend.ts 的 fileName 保持一致，避免在 Worker 中引入
// Tauri 运行时依赖（后者引用 window，无法在 Worker 内使用）。
...
export function normalizeFrameMatches(record: Record<string, unknown>, threshold: number): ReportFrameMatch[] {
