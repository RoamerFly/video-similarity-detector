...
Options:
  ...
  --no-stop-running-app      Fail instead of stopping an old packaged app in dist_macos.
  ...
  if ! command -v "$1" >/dev/null 2>&1; then
    ...
    case "$1" in
      rustc|cargo)
        ...
        ;;
      node|npm)
        echo "        Install Node.js LTS with Homebrew or from https://nodejs.org/." >&2
        ;;
    esac
    exit 1
  fi
...
copy_macos_artifact() {
  if [[ ! -f "$APP_BINARY" ]]; then
    echo "[ERROR] No macOS build artifact found: $APP_BINARY" >&2
    exit 1
  fi

  cp "$APP_BINARY" "$DIST_DIR/video-similarity-desktop"
  chmod +x "$DIST_DIR/video-similarity-desktop"
  cat > "$DIST_DIR/run-video-similarity.command" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"
export USE_TF=0
...
EOF
  chmod +x "$DIST_DIR/run-video-similarity.command"
}

write_readme() {
  cat > "$DIST_DIR/README_macos.txt" <<'EOF'
Video Similarity - macOS portable package
...
echo
echo "========================================"
echo "  Video Similarity - macOS Packager"
echo "========================================"
...
if [[ ! -x "$ENV_DIR/ffmpeg" || ! -x "$ENV_DIR/ffprobe" ]]; then
  ...
  case "$(uname -m)" in
    arm64) ffmpeg_platform="macos-arm64" ;;
    x86_64) ffmpeg_platform="macos-x64" ;;
    *) echo "[ERROR] Unsupported macOS architecture: $(uname -m)" >&2; exit 1 ;;
  esac
  "$REPO_ROOT/scripts/prepare-ffmpeg-runtime.sh" "$ffmpeg_platform"
fi
